...
static void terminal_enable_raw_mode(void) {
    ...
    // We will block until 1 bytes has been read */
...
void history_add_entry(InputEntry *entry) {
    ...
    if (history_state.newest_entry_index == HISTORY_NONE_SELECTED_MARK) {
        ...
        history_state.newest_entry_index = (history_state.newest_entry_index + 1) % HISTORY_MAX_ENTRIES;
    }
...
#define DEBUG_HIST_ADD_ENTRY(value) history_add_entry(&(InputEntry){.buffer = (value), .length = strlen((value))})
...
int main(void) {
    ...
    DEBUG_HIST_ADD_ENTRY("2. I overwrote the first entry!");
    DEBUG_HIST_ADD_ENTRY("3. three");
    DEBUG_HIST_ADD_ENTRY("4. four");
    DEBUG_HIST_ADD_ENTRY("5. four");
